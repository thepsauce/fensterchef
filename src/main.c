...
void init_connection(void)
{
    ...
    if (xcb_connection_has_error(g_dpy) > 0) {
        fprintf(stderr, "could not create xcb connection\n");
        exit(1);
    }
}
...
int take_control(int screen_no)
{
    xcb_screen_t        *screen;
    xcb_generic_error_t *error;

    screen = g_screens[screen_no];
    g_screen_no = screen_no;
...
void accept_new_window(xcb_drawable_t win, xcb_screen_t *screen)
{
...
void handle_key_press(xcb_key_press_event_t *ev)
{
    ...
    for (uint32_t i = 0; i < SIZE(keys); i++) {
        if (keysym == keys[i].keysym && keys[i].mod == ev->state) {
            fprintf(stderr, "triggered keybind: %d\n", keys[i].action);
...
void handle_event(void)
{
    ...
    if (ev != NULL) {
        ...
        case XCB_MAP_REQUEST:
            accept_new_window(((xcb_map_request_event_t*) ev)->window, g_screens[0]);
            break;
